...
- 42 of the 256 legal `usage_stats` species (Aerodactyl, Kangaskhan, Aegislash, Starmie, Drampa, etc.) have no `pokemon` table row via the real resolver — see `scripts/check_missing_species.js`, this is the exact source of the "N skipped for missing species data" bounds line.
- `seed_learnsets.js`'s `isNonstandard` gate now seeds a species if it's observed in `tournament_teams` (fixed — unblocked 62 Mega forms that were ALSO silently excluded, since every Mega form is `isNonstandard` in `@pkmn/dex`). Aegislash and Floette-Eternal-Mega, the two cases that motivated the fix, are STILL uncovered — Aegislash has no `pokemon` row at all (above); Floette-Eternal-Mega doesn't resolve to its own dex learnset (fuzzy-matches to the unrelated Floette-Mega). See `DEFERRED.md` #1.
- `archetype_matchups.js`'s `buildArchetypeMeta()` picks item/ability/moves for ARCHETYPE MATCHUPS "key threats" via the same independent-argmax pattern `candidateProfile()` used to (now fixed there). Not fixed here — no coherence check exists for this function yet. See `DEFERRED.md` #2.
- `src/tests/api.test.js`'s Venusaur threshold-attribution test now fails deterministically (not flaky) since the GA was seeded — a real spread where the hardest threat ties HP and SpD. See `DEFERRED.md` #6.
