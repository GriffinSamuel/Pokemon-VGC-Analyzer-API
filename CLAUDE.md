...
- **NOTE (2026-08-31, weather_labels session):** the ARCHETYPE MATCHUPS text section described in several entries above (`buildExchangeGrid`, `damageVerdict()`, the exchange grid's weather/Tailwind assumptions) is **no longer rendered by `POST /api/team/build`** — see the two "Fixed this session" entries below. The entries above remain accurate descriptions of bugs fixed IN `archetype_matchups.js`'s code, which still exists in the repo unmodified; they are simply no longer reachable from this route. `damageVerdict()` itself was deleted from `team.js` as dead code once its only caller (the ARCHETYPE MATCHUPS renderer) was removed.
- **Fixed this session:** `scoreSpread()`'s DEFENSIVE branch never set `primary_weather`/`alt_weathers` on its `thresholds_met` entries the way the OFFENSIVE branch already did — so a defensive Why-block threshold for a weather-sensitive move (e.g. a Water-move HP threshold) printed an unlabelled number computed under one weather while a live alternate weather elsewhere on the board could make it 3x worse (verified live: Charizard-Mega-Y's "15 HP — survives Swampert-Mega Wave Crash" read as an unconditional survival under our own Sun at 82.1-97.6%, while the exact same hit under our own Pelipper's live Rain is an OHKO at 251.2-296.4% — the 251.2-296.4% figure exactly matches this file's own prior "observed failure" example above, from before any of that grid was deleted). Added a shared `src/utils/weather_rules.js` (`weatherChangesDamage`/`resolveTypeFor`) consolidating what turned out to be THREE separate local copies of the same "does weather change this move" predicate (`archetype_matchups.js`, and a third one discovered mid-fix in `spread_scorer.js`'s own `displayMoveName`) — extended with Utility Umbrella item-awareness and a new accuracy-note helper for Thunder/Hurricane/Blizzard. The OFFENSIVE branch's alt-weather computation was also retrofitted onto the same predicate (it previously ran ungated whenever the team had multiple weathers, regardless of whether the move was weather-sensitive at all). `team.js`'s `describeThresholdForWhy()` now renders the new fields on both the primary Why-block line and every `[also: ...]` secondary interaction, and a new `buildWeatherSummary()` states the team's live weathers and each member's assumed weather once near the top of the report, so an untagged figure has a stated meaning (weather cannot change it) rather than silence meaning "not considered." **Second-layer bug found while verifying the fix:** `weightedDefensiveDamage()`'s memoization cache key never included weather at all — dormant until this session's own new alt-weather feature became the first caller to invoke it twice for the same attacker/move/spread/defender-stats under two different weathers, at which point the alt call silently returned the primary call's cached result relabeled as the alternate. Fixed by adding `fieldOpts?.weather` to the cache key, matching `weightedOffensiveDamage()`'s existing pattern.
- **Fixed this session (owner decision):** the ARCHETYPE MATCHUPS text section (exchange grid, key threats, resistances, counters, and — since they were nested per-archetype inside this same block, not a separate section — Possible Swaps and Best Team Set) is removed from `POST /api/team/build`'s text output entirely. `analyzeArchetypeMatchupsLive()` is no longer called by this route at all: `team_score` dropped from a 3-signal to a 2-signal average (coverage + synergy; the archetype-favorable-ratio signal removed), and `archetype_matchups` is no longer part of the JSON response body. `archetype_matchups.js`/`archetype_swaps.js` remain in the repo, behavior unmodified, just uncalled from this one route — no other route currently calls them either, so this analysis is presently unreachable from any live endpoint. Two now-orphaned `api.test.js` tests (Speed-ceiling and Best-Team-Set-Mega-count invariants, both keyed on the now-absent `body.archetype_matchups`) were removed with an explanatory comment rather than silently deleted; both invariants are still real and worth re-adding if this analysis is ever wired back into a live endpoint.
